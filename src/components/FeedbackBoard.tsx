...
const defaultCriteria: Record<string, { name: string; criteria: { name: string; maxScore: number; description: string }[] }> = {
  '이메일': {
    name: '이메일',
    criteria: [
      { name: '구조 완결성', maxScore: 25, description: '제목·인사말·용건·마무리·서명' },
      { name: '수신자 맞춤 어조', maxScore: 25, description: '격식체·존댓말 일관성' },
      { name: '용건의 명확성', maxScore: 25, description: '첫 문단 목적 명확성' },
      { name: '형식적 무결점', maxScore: 25, description: '오탈자·띄어쓰기·맞춤법' }
    ]
  },
  '초청장': {
    name: '초청장',
    criteria: [
      { name: '행사 정보 완결성', maxScore: 25, description: '행사명·일시·장소·주최자' },
      { name: '초청 목적 명확성', maxScore: 25, description: '초청 이유 명확성' },
      { name: '정중한 어조', maxScore: 25, description: '격식있는 요청 표현' },
      { name: '형식적 무결점', maxScore: 25, description: '오탈자·날짜·연락처 오류' }
    ]
  },
  '부고문': {
    name: '부고문',
...
export default function FeedbackBoard({ student, history, onHistoryChange, onGoAdmin, onLogout }: Props) {
  ...
  const buildSystemInstruction = () => {
    ...
    // Get current criteria profile
    let currentProfile = currentAssignment ? {
      name: currentAssignment.name,
      criteria: currentAssignment.criteria,
      special: currentAssignment.specialInstruction
    } : null;

    // If no assignment selected, try to guess or use default "이메일" as default fallback if needed
    // But better to just provide default criteria for common types
    if (!currentProfile) {
       // We'll use a template that says "Choose a type or I will help you with Email/Invitation/Obituary"
    }
...
[과도한 지적 방지 규칙]
오류가 확실한 경우에만 지적한다. 애매하거나 문체 차이일 수 있는 경우 지적하지 않는다.
판단 기준:
- 명백한 오탈자 → 지적 ✅
- 명백한 맞춤법 오류 → 지적 ✅
- 필수 구성요소 누락 → 지적 ✅
- 문장이 어색하지만 틀린 것은 아닌 경우 → 지적 ❌
- 여러 표현 방식 중 하나인 경우 → 지적 ❌
- 개인 문체 차이인 경우 → 지적 ❌
확신이 없으면 지적하지 않는다. "이런 표현도 있을 수 있지 않나요?" 같은 불필요한 질문을 하지 않는다.
오류가 없는 부분은 반드시 명확하게 인정한다. "이 부분은 적절하게 작성되었습니다."

[현재 과제 유형]
${currentProfile ? currentProfile.name : "비즈니스 문서 (이메일/초청장/부고문)"}

[평가 지표 및 배점]
${currentProfile 
  ? currentProfile.criteria.map((c, i) => `${i+1}. ${c.name} (${c.maxScore}점): ${c.description}`).join('\n')
  : `
1. 구조 완결성 (25점): 제목·인사말·용건·마무리·서명이 모두 있는가
2. 수신자 맞춤 어조 (25점): 수신 대상에 맞는 격식체·존댓말이 일관되는가
3. 용건의 명확성 (25점): 첫 문단에 목적이 명확히 드러나는가
4. 형식적 무결점 (25점): 오탈자·띄어쓰기·맞춤법 오류가 없는가
  `.trim()}

${(currentProfile?.special || commonInstruction) ? `[특별 지시사항]\n${commonInstruction}\n${currentProfile?.special || ""}` : ""}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[절대 원칙 - 반드시 준수]
1. 수정된 문장이나 '더 나은 표현'을 절대 직접 써주지 않는다. (모범 답안 제공 금지)
2. 학생이 직접 생각하게 하기 위해, 오류 발견 시 반드시 "질문 형태"로만 피드백한다.
3. "이런 식으로 쓰면 어떨까요?"라며 수정된 문장을 예시로 주는 것도 절대 금지다.
4. 오류의 위치(X번째 줄)와 어떤 종류의 오류인지만 구체적으로 명시한다.
5. 한 번에 우선순위 높은 것 3~5개 정도를 위주로 지적한다.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

[매우 중요: 내용 확인 불가 처리]
만약 제공된 텍스트나 이미지/PDF에서 문서의 내용을 전혀 찾을 수 없거나 글자를 읽을 수 없는 경우,
...
[점수]
${currentProfile 
  ? currentProfile.criteria.map((c, i) => `지표${i+1}:XX`).join('\n')
  : `지표1:XX\n지표2:XX\n지표3:XX\n지표4:XX`}
[/점수]

(참고: XX는 해당 지표의 획득 점수입니다. 배점을 넘을 수 없습니다.)
    `;
  };
  ...
  const handleSuccess = async (data: FeedbackResponse, textUsed: string, fileUsed: File | null) => {
    ...
    const gasData: any = {
      ...
      timestamp: new Date().toISOString(),
      score1: data.scores.items[0]?.score || 0,
      score2: data.scores.items[1]?.score || 0,
      score3: data.scores.items[2]?.score || 0,
      score4: data.scores.items[3]?.score || 0,
  ...
  const requestFeedback = async (text: string, file: File | null) => {
    ...
    // Parse scores using new [점수] tag
    const scoreTagMatch = feedbackText.match(/\[점수\](.*?)\[\/점수\]/s);
    const currentAssignment = activeAssignments.find(a => a.id === selectedAssignmentId);
    
    // Determine which criteria set was used
    let activeCriteria = currentAssignment 
      ? currentAssignment.criteria 
      : (defaultCriteria['이메일'].criteria); // Default fallback

    // If automatic mode, we might need more logic, but for now fallback to defaults or first active
    if (!currentAssignment && activeAssignments.length > 0) {
       // Maybe the AI guessed the type? Let's check feedback text
       if (feedbackText.includes("초청장")) activeCriteria = defaultCriteria['초청장'].criteria;
       else if (feedbackText.includes("부고문")) activeCriteria = defaultCriteria['부고문'].criteria;
    }
    ...
    if (scoreTagMatch) {
      const tagContent = scoreTagMatch[1];
      activeCriteria.forEach((c, idx) => {
        const lineMatch = tagContent.match(new RegExp(`지표${idx+1}:\\s*(\\d+)`));
        const score = lineMatch ? parseInt(lineMatch[1]) : 0;
        items.push({
          name: c.name,
          score: score,
          maxScore: c.maxScore
      ...
      // Fallback if AI missed the tag
      activeCriteria.forEach(c => {
        items.push({ name: c.name, score: 0, maxScore: c.maxScore });
      });
    }

    const cleanedFeedback = feedbackText.replace(/\[점수\].*?\[\/점수\]/gs, "").trim();
    let docType = currentAssignment ? currentAssignment.name : "기타";
    
    if (!currentAssignment) {
       if (cleanedFeedback.includes("이메일") || text.includes("이메일")) docType = "이메일";
       else if (cleanedFeedback.includes("초청장") || text.includes("초청장")) docType = "초청장";
       else if (cleanedFeedback.includes("부고문") || text.includes("부고문")) docType = "부고문";
    }

    return {
      feedback: cleanedFeedback,
      scores: { totalScore, items },
      docType
    };
  };
                        ...
                        <div className="flex items-start gap-2 text-[#cc0000]">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-[10px] leading-snug font-medium opacity-90">
                              ...
                              반드시 같은 문서를 2회 이상 제출하여 피드백 결과를 교차 확인하세요. 
                              AI는 때로 불필요한 수정을 요구할 수 있습니다. 최종 판단은 교수자의 평가기준에 따릅니다.
                            </p>
                          </div>
                        </div>
                        <button
                    ...
                    {item.scores && (
                      <div className={`mt-4 overflow-hidden rounded-xl border-2 ${
                        ...
                        <div className="p-4 space-y-2">
                          {item.scores.items ? (
                            item.scores.items.map((scoreItem, idx) => (
                              <div key={idx} className="flex justify-between items-center text-xs font-medium text-gray-600">
                                <span>{idx + 1}. {scoreItem.name}</span>
                                <span className="font-bold">{scoreItem.score} / {scoreItem.maxScore}점</span>
                              </div>
                            ))
                          ) : (
                            <p className="text-[10px] text-gray-400">점수 항목을 불러올 수 없습니다.</p>
                          )}
                        </div>
                        <div className={`px-4 py-3 border-t flex items-center justify-between font-black ${
                          ...
                          <span className="text-xl">{item.scores.totalScore} / {item.scores.items?.reduce((sum, i) => sum + i.maxScore, 0) || 100}점</span>
                        </div>
                      </div>
                    )}
                  </div>
