...
export default function AdminView({ onBack }: Props) {
  ...
  const formatTimestamp = (ts: any) => {
    if (!ts) return '-';
    // If GAS returns it as a Date object or ISO string
    ...
    // If it's a string that Date can't parse directly, just return it sanitized
    return String(ts).split('GMT')[0].trim();
  };
  ...
  const filteredData = (data || []).filter(item => {
    ...
    return name.includes(sTerm) || id.includes(sTerm) || type.includes(sTerm);
  });
